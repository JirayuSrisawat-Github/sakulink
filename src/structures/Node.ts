  ...
  protected async message(d: Buffer | string): Promise<void> {
    ...
    switch (payload.op) {
      ...
      case "ready":
        ...
        this.manager.db.set(
          `sessionId.${
            this.options.identifier ?? this.options.host.replace(/\./g, "-")
          }`,
          this.sessionId
        );
        ...
        for (const resumedPlayer of resumedPlayers) {
          const previousInfosPlayer: any =
            this.manager.db.get(`players.${resumedPlayer.guildId}`) || {};
          ...
          const track = await this.manager.decodeTrack(
            previousInfosPlayer.current.track
          );
          ...
          previousInfosPlayer.queue.map(async (queue: TrackData) =>
            player.queue.add(TrackUtils.build(queue))
          );
        }
        break;
      default:
        ...
        this.manager.emit(
          ...
          this,
          new Error(
            `Unexpected op "${payload.op}" with data: ${payload.message}`
          )
        );
        return;
    }
